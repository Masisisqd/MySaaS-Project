...
export const createFamilyFn = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
...
export const registerChildFn = functions.https.onCall(async (data, context) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        const { hashedSecretPin, salt } = familySecretsSnap.data() as any;
...
export const loginChildFn = functions.https.onCall(async (data, context) => {
    ...
    const { hashedPin, salt } = childSecretSnap.data() as any;
...
export const getFamilyProfilesFn = functions.https.onCall(async (data, context) => {
    const { familyId } = data;
